    ...
    stages {
        ...
        stage("Test") {
            steps {
                ...
                sh "docker exec tortoisebot-ros2-gazebo bash -c 'source /opt/ros/galactic/setup.bash; source /ros2_ws/install/setup.bash; colcon test --packages-select tortoisebot_waypoints --event-handler=console_direct+'"
            }
        }
        stage("Clean") {
            steps {
